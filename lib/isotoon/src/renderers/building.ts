...
const PALETTE = [0xE57373, 0xFFB74D, 0xFFF176, 0x81C784, 0x64B5F6, 0xBA68C8, 0xF06292, 0x4FC3F7]

export function renderBuildings(features: Building[], container: Container, scale: number) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    drawBuilding(g, building, scale, baseColor)
...
function drawBuilding(g: Graphics, building: Building, scale: number, color: number) {
  ...
  for (let i = 0; i < base.length - 1; i++) {
    ...
    walls.push({ quad, depth, color: sideShade(color, a, b) })
  }
